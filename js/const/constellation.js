...
$(function() {
	...
	var ConstellationApp = function() {
		var self = this;
		
		self.win = $(window);
		self.toolbar = $('.const-toolbar');
		self.properties = $('.const-properties');
		self.canvas = $('.const-canvas');
		
		self.win.on('resize', function() {
			self.resize();
		}).on('keydown', function(evt) {
			self.keypress( evt.which );
		});
		
		self.canvas
			.css('margin-top', self.toolbar.height() + 10)
			.css('margin-bottom', self.properties.height() + 10);
		
		self.setSize();
	};
	ConstellationApp.prototype = {
		height: 0,
		width: 0,
		...
		resize: function() {
			if (!this.height) {
				this.ui.height( this.win.height()-parseInt(this.ui.css('margin'), 10)*2 );
			} else {
	...
	var Const = new ConstellationApp();
});
